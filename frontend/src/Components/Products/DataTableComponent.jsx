...
const DataTableComponent = () => {
    ...
    const categoriesRow = products.map((item, index) => ({
        ...
        status: <span style={{backgroundColor:`${item.status==='notAvailble'?'red':'green'}`,color:'white',padding:'0px 15px',borderRadius:'17px'}}>{item.status==='notAvailble'?'Not Availble':item.status==='available'?'Available':''}</span>,
        Action: <>
